...
class ArcMarginProduct(nn.Module):
    ...
    def __init__(
        ...
        self.cos_m = torch.cos(m).to(device=device)
        self.sin_m = torch.sin(m).to(device=device)
        self.th = torch.cos(math.pi - m).to(device=device)
        self.mm = (torch.sin(math.pi - m) * m).to(device=device)
...
class LitModule(pl.LightningModule):
    ...
    def training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        if self.current_epoch > 3 and self.unfreeze_flag:
...
def main():
    ...
    for val_fold in range(n_fold):
        ...
        if is_train:
            ...
            value_counts = train_df["individual_id"].value_counts().sort_index()
