...
from src.models.dolg import DolgNet
...
class LitModule(pl.LightningModule):
    def __init__(
        ...
        super().__init__()
        self.__build_model(
            model_name, pretrained, embedding_size, num_classes, arc_s, arc_m, arc_easy_margin, arc_ls_eps, image_size
        )
        self.save_hyperparameters()
...
def main():
    ...
    for val_fold in range(n_fold):
        ...
        if is_infer:
            model_path = list(CHECKPOINTS_DIR.glob(f"{model_name}_{image_size}_fold{val_fold}*.ckpt"))
            print(model_path)
            infer(
