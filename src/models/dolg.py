...
class MultiAtrous(nn.Module):
    def __init__(self, in_channel: int, out_channel: int, size: int, dilation_rates: List[int] = [3, 6, 9]) -> None:
        super().__init__()
        self.dilated_convs = [
            nn.Conv2d(
                in_channel,
...
class DolgLocalBranch(nn.Module):
    def __init__(self, in_channel: int, out_channel: int, img_size: int, hidden_channel: int = 2048) -> None:
        super().__init__()
        self.multi_atrous = MultiAtrous(in_channel=in_channel, out_channel=out_channel, size=int(img_size / 8))
...
class DolgNet(nn.Module):
    def __init__(
        ...
        self.local_branch = DolgLocalBranch(in_channel=512, out_channel=hidden_dim, img_size=img_size)
