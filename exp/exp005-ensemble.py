...
def train(
    ...
    model_checkpoint = ModelCheckpoint(
        checkpoints_dir, filename=f"{model_name}_{image_size}", monitor="val_loss", save_weights_only=True
    )
...
def infer(
    ...
    for idx, checkpoint_path in enumerate(checkpoint_paths):
        print(f" ### loaded model path: {checkpoint_path}")
        module = load_eval_module(checkpoint_path, torch.device("cuda"))

        train_image_names, train_embeddings_, train_targets = get_embeddings(module, train_dl, encoder, stage="train")
        val_image_names, val_embeddings_, val_targets = get_embeddings(module, val_dl, encoder, stage="val")
        test_image_names, test_embeddings_, test_targets = get_embeddings(module, test_dl, encoder, stage="test")

        train_embeddings.append(train_embeddings_)
        val_embeddings.append(val_embeddings_)
        test_embeddings.append(test_embeddings_)
    ...
    val_embeddings = np.concatenate(val_embeddings, axis=1)
    test_embeddings = np.concatenate(test_embeddings, axis=1)

    D, I = create_and_search_index(
        module.hparams.embedding_size * len(checkpoint_paths), train_embeddings, val_embeddings, k
    )  # noqa: E741
    ...
    D, I = create_and_search_index(
        module.hparams.embedding_size * len(checkpoint_paths), train_embeddings, test_embeddings, k
    )  # noqa: E741
    print("Created index with train_embeddings")
...
def main():
    ...
    n_fold = 5
    max_epochs = 10
    warm_start_path = "output/checkpoints/convnext_base_384_in22ft1k_256.ckpt"
    ...
    for val_fold in range(n_fold):
        ...
        if is_infer:
            # model_path = list(CHECKPOINTS_DIR.glob(f"{model_name}_{image_size}*.ckpt"))
            model_paths = [
                "output/checkpoints/convnext_base_in22ft1k_128.ckpt",
                "output/checkpoints/convnext_base_384_in22ft1k_256.ckpt",
                "output/checkpoints/convnext_base_384_in22ft1k_384-v1.ckpt",
            ]
            infer(
