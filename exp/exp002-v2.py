...
config: DictConfig = OmegaConf.create(
    dict(
        exp_desc="exp002-batch512-emb512-convnext-small-remove-back-aug-v2-newlabel-seg",
        train=True,
        debug=False,
        train_fold=[0, 1, 2, 3, 4],
        inference=True,
        device="cuda" if torch.cuda.is_available() else "cpu",
        seed=2022, img_size=384,
        n_splits=5,
        num_classes=15587,
        ...
        scheduler=dict(
            ...
            # name="optim.lr_scheduler.CosineAnnealingLR", params=dict(T_max=30),
        ),
...
def get_df(config: DictConfig, mode: str = "train") -> pd.DataFrame:
    ...
    if mode == "train":
        ...
        test_img_dir = Path("./input/happy-whale-and-dolphin-backfin") / "test_images"
        ...
        test_df = pd.read_csv(Path("./input/happy-whale-and-dolphin-backfin") / "sample_submission.csv")
        ...
        logger.info(f"test_df.head = {test_df.head()}")
        return test_df
...
# infer funcs
# ==================
def get_predictions(df: pd.DataFrame, threshold: float = 0.2) -> Dict[str, List[str]]:
...
def load_eval_module(checkpoint_path: str, device: torch.device, config: DictConfig):
...
def visualize_aug_image(dataloader, config, mode):
    ...
    for i in range(5):
        for j in range(5):
            ...
            img = img.astype("i4")
            ax[i, j].imshow(img)
            ax[i, j].set_title(str(Path(b["image_name"]).name))
            ax[i, j].set_axis_off()
...
def get_embeddings(
    ...
    for batch in tqdm(dataloader, desc=f"Creating {mode} embeddings"):
        image_names = batch["image_name"]
        images = batch["image"].to(module.device)
        targets = batch["label"].to(module.device)

        _, embeddings = module(images, targets)
        embeddings = embeddings.cpu().numpy()
        np.nan_to_num(embeddings, copy=False)
    ...
    all_targets = np.concatenate(all_targets).astype("i4")
...
def create_distances_df(
    image_names: np.ndarray, targets: np.ndarray, D: np.ndarray, I: np.ndarray, mode: str
) -> pd.DataFrame:
    distances_df = []
    for i, image_name in tqdm(enumerate(image_names), desc=f"Creating {mode}_df"):
        target = targets[I[i]]
        distances = D[i]
...
class HappyWhaleDataset(Dataset):
    def __init__(self, df: pd.DataFrame, mode: str, transform: Optional[dict] = None) -> None:
        ...
        self._transform = timm.data.transforms_factory.create_transform(
            input_size=(config.img_size, config.img_size), crop_pct=1.0
        )
...
class MyLitModel(pl.LightningModule):
    ...
    def training_step(self, batch: dict, batch_idx: int) -> dict:
        ...
        loss, pred, labels, img_name, idx, emb = self.__share_step(batch, "train")
        return {
            "loss": loss,
            "pred": pred,
            "labels": labels,
    ...
    def validation_step(self, batch: dict, batch_idx: int) -> dict:
        loss, pred, labels, img_name, idx, emb = self.__share_step(batch, "val")
        return {
            "pred": pred,
            "labels": labels,
            "image_name": img_name,
    ...
    def __share_step(self, batch: dict, mode: str):
        ...
        img_path, idx = batch["image_name"], batch["id"]
        if mode == "train":
    ...
    def test_step_end(self, outputs):
        ...
        image_paths = np.concatenate([out["image_name"] for out in outputs])
...
def infer(
    ...
    module = load_eval_module(checkpoint_path, torch.device("cuda"), config)
    ...
    D, I = create_and_search_index(module.hparams.embedding_size, train_embs, val_embs, k)

    val_targets_df = create_val_targets_df(
        train_targets=train_labels, val_image_names=val_img_name, val_targets=val_labels
    )
    logger.info(f"val_targets_df.head() = \n {val_targets_df.head()}")

    val_df = create_distances_df(image_names=val_img_name, targets=train_labels, D=D, I=I, mode="val")
    logger.info(f"val_df.head() = \n {val_df.head()}")
    ...
    train_embs = np.concatenate([train_embs, val_embs], axis=0)
    ...
    D, I = create_and_search_index(
        embedding_size=module.hparams.embedding_size, train_embeddings=train_embs, val_embeddings=test_embs, k=k
    )
    test_df = create_distances_df(image_names=test_img_name, targets=train_labels, D=D, I=I, mode="test")
